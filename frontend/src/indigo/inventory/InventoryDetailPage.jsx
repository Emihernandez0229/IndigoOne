...
export default function InventoryDetailPage() {
  ...
  const isOwner =
    currentUser?.role === ROLES.INDIGO_SUPER_USUARIO ||
    currentUser?.role === ROLES.INDIGO_DUENO;
  ...
  if (!item) {
    return (
      <PageContainer title="Producto" description="">
        ...
        />
        <div className="mt-4">
          <Link to="/indigo/inventario" className="text-sm font-medium text-indigo-primary">
            ← Volver a inventario
          </Link>
        </div>
      </PageContainer>
  ...
  return (
    ...
    <PageContainer
      ...
      actions={
        <Button variant="outline" className="inline-flex items-center gap-2 py-2.5 text-sm" onClick={() => navigate("/indigo/inventario")}>
